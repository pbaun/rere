...
object ReqlObjectProducer {
  ...
  implicit def reqlNullToObjectProducer[SourceType <: ReqlDatum, TargetType <: ReqlObject](value: ReqlNull): ReqlObjectProducer[SourceType, TargetType] =
    new ProxyQuery(value) with ReqlObjectProducer[SourceType, TargetType]

  implicit def objectToObjectProducer[SourceType <: ReqlDatum, TargetType <: ReqlObject](value: JsonObject): ReqlObjectProducer[SourceType, TargetType] =
    new ProxyQuery(values.expr(value)) with ReqlObjectProducer[SourceType, TargetType]

  implicit def mapToObjectProducer[SourceType <: ReqlDatum, TargetType <: ReqlObject](value: Map[String, ReqlDatum]): ReqlObjectProducer[SourceType, TargetType] =
    new ProxyQuery(values.expr(value)) with ReqlObjectProducer[SourceType, TargetType]
